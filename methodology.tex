...
Simultaneous learning of multiple policies is possible because state-transition dynamics provides enough information for more than one policy to be successfully updated. As an example consider state-transition depicted on Figure~\ref{fig:reasonForMPLearning}. Let $ \vec{w}_{1}=\{0.7,0.3\} $  and $ \vec{w}_{2}=\{0.3,0.7\} $ be two weight vectors. Each vector represents relative importance of objectives. Weight vectors $ \vec{w}_{1} $  and $ \vec{w}_{2} $ will get mapped to policies $ \pi_{\vec{w}_{1}} $  and $ \pi_{\vec{w}_{2}} $ respectively. Let $ \pi_{\vec{w}_{1}} $  and $ \pi_{\vec{w}_{2}} $ be deterministic policies.

Further let $ \pi_{\vec{w}_{1}} $ be an active policy. At time step $ t $ action-selection was made according to active policy $ \pi_{\vec{w}_{1}} $ and let $ a_{t}=left $. Pair $ (s_{t}, a_{t}) $ makes the environment to transition to state $ s_{t+1} $ and the immediate reward [20,20] is received by the learning algorithm.
...
At time step $ t+1 $ it is time to update $ Q(s_{t}, a_{t}) $ for both $ \pi_{\vec{w}_{1}} $  and $ \pi_{\vec{w}_{2}} $. To update $ Q(s_{t}, a_{t}) $ the algorithm will need to identify greedy action for the state $ s_{t+1} $ and use Q-value of that greedy action in combination with the immediate reward. For each policy the greedy action might be different but the immediate reward will be the same. This allows to update Q-function for the policy $ \pi_{\vec{w}_{2}} $ even though the state-transition was generated by the policy $ \pi_{\vec{w}_{1}} $.

Table \ref{table:Qfunctionsfortplusone} shows shows the Q-functions for the state $ s_{t+1} $ for both policies $ \pi_{\vec{w}_{1}} $  and $ \pi_{\vec{w}_{2}} $. These two Q-functions are identical because both actions lead to the terminal state and only immediate reward is used during the update. For each policy the greedy action is the action which will maximize the weighted sum:

$$ \displaystyle\sum_{i=1}^{n} w_{i}f_{i}(a). $$

For the policy $ \pi_{\vec{w}^{1}} $ the weight vector is $ \vec{w}^{1}=\{0.7,0.3\} $ and thus the greedy action will be $ left $ because

$$ (60 \times 0.7) + (40 \times 0.3) > (40 \times 0.7) + (60 \times 0.3). $$

On the other hand for the policy $ \pi_{\vec{w}^{2}} $ the weight vector is $ \vec{w}^{2}=\{0.3,0.7\} $ and the greedy action will be $ right $ because

$$ (60 \times 0.3) + (40 \times 0.7) < (40 \times 0.3) + (60 \times 0.7). $$

Thus the update rule for $ Q^{ \pi^{\vec{w}^{1}} }(s_{t},a_{t}) $ will take the following form:
...
And the update rule for $ Q^{ \pi^{\vec{w}^{2}} }(s_{t},a_{t}) $ will take the following form:
...
\begin{table}[!htb]
    \caption{\textbf{Left-hand side table} is the Q-function for the state $s_{t+1}$ for the policy $ \pi_{\vec{w}_{1}} $. \textbf{Right-hand side table} is the Q-function for the state $s_{t+1}$ for the policy $ \pi_{\vec{w}_{2}} $.}
